...
<title>sent emails - user control panel - phemi (TM) - php e-mailer interface</title>
<script>
...
<body>
<div id="bx" style="display:none;border:1px solid black;width:400px;height:110px;position:absolute;background:#666666;padding:5px; left:70px; top:15px">
...
$del=ge("delete");
if (mysql_query("delete from sent where id='$del'")) {$done=1;}
else {$done=0;}
}
}
$usent=ro(mysql_query("select * from sent where owner='$userid' order by id desc"));
?>
<p><a href="home.php">home</a> | <b><font color="#666666">sent (<?=$usent;?>)</font></b> | 
<a href="failed.php">failed (<?=$ufailed;?>)</a> | 
<a href="options.php">options</a> | 
<a href="index.php?logout">logout</a></p>
<?
if (isset($_GET['id'])) {
$id=ge("id");
$a=mysql_query("select * from sent where id='$id'");
$email=re($a,0,'to');
$name=re($a,0,'name');
$to=re($a,0,'to');
...
<input type="reset" value="cancel" name="submit" class="button" onclick="top.location='sent.php'"></td>
</tr>
</table>
</form>
</td>
...
<table border="1" width="500" id="table3" cellpadding="0" style="border-collapse: collapse">
<tr class="header">
<td width="50%" valign="top">sent emails</td>
</tr>
<tr>
<td width="50%" valign="top">
...
<?
for ($i=1;$i<=$usent;$i++) {
$subject=re($b,$i-1,'subject');
$id=re($b,$i-1,'id');
$to=re($b,$i-1,'to');
$recp=count(split(",",$to));
echo "<td>".($usent-$i+1)."</td><td><a href='sent.php?delete=$id' onclick=\"if (!confirm('are you sure you want to delete this email?')) {return false}\"><img src='../admin/images/delete.gif' border='0' title='delete this email' /></a></td><td><a href='sent.php?id=$id'>$subject</a></td><td>$recp</td></tr>";
}
